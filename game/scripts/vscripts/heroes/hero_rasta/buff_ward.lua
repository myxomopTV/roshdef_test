...
-----------------------------------------------------------------------------
function rasta_buff_ward:OnAbilityPhaseStart()
	local caster = self:GetCaster()
	local target = self:GetCursorTarget()
	local target_owner = target:GetOwner()

	if caster == target_owner then
		return true
	else
		return false
	end
...
function rasta_buff_ward:OnSpellStart()
	if IsServer() then
...
function rasta_buff_ward:OnChannelFinish( bInterrupted )
	if IsServer() then
